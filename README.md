...
2. 按 [CONTRIBUTING.md](CONTRIBUTING.md) 实现 Provider，并通过 Pull Request 提交。
3. 首次适配合并后，贡献者会加入贡献者名单。
4. 愿意持续处理该校认证变更和故障的贡献者，将登记为学校维护者，并由仓库所有者邀请为 GitHub 协作者。

成为协作者后仍建议通过分支和 Pull Request 修改代码，避免未经审核的改动影响其他学校。
...
学校适配、问题修复、测试或文档贡献合并后，可以加入贡献者名单。愿意持续负责某所学校认证适配的人，可以申请登记为学校维护者，并由仓库所有者邀请为 GitHub 协作者。
