...
def encrypt_password(password):
    """使用页面硬编码的公钥加密密码（通过直接调用 Node.js）"""

    # 1. 关键：polyfill 一个假的 window 对象，让 security.js 能正常执行
    # 注意：这里去掉了 console 的 polyfill，因为我们需要用它来向 Python 返回结果
    polyfill = """
    var window = this || {};
    ...
    # 2. 调用函数定义（注意去掉了 f-string 以免大括号与 JS 语法冲突）
    call_js = """
    ...
    # 3. 使用 json.dumps 安全处理密码字符串，防止包含引号导致 JS 报错
    # 并在脚本最后调用 console.log 输出结果
    safe_password = json.dumps(password)
    execute_js = f"\nconsole.log(getEncrypted({safe_password}));\n"

    # 4. 拼接完整代码
    full_js = polyfill + get_js_code() + call_js + execute_js

    # 5. 使用 subprocess 调用 Node 进程
    try:
        # 通过 input 参数将 js 代码直接输入到 stdin，避免了写临时文件的 IO 开销
        process = subprocess.run(
            ["node"],
            input=full_js,
            text=True,  # 自动处理字符串的编码/解码
            capture_output=True,  # 捕获 stdout 和 stderr
            encoding="utf-8",  # 显式指定编码
            check=True,  # 如果 Node 退出码非 0 则抛出异常
        )
        ...
        print(f"Node.js 执行出错，错误信息：\n{e.stderr}")
        return None
    except FileNotFoundError:
        print("未找到 Node.js，请确认 Node 已正确安装并配置到了环境变量(PATH)中。")
        return None
